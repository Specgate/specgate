      ...
      <div className="p-6 space-y-5">
        <div className="rounded-xl border border-primary/30 bg-primary/5 p-4">
          ...
          <p className="text-sm text-muted-foreground">
            3 specs are ready for agent handoff. REQ-004 is approved but has no developer assigned.
            REQ-002 is ready for stakeholder preview after expiry mismatch is resolved.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-4">
          {cols.map((col) => {
            const items = col.key === "assigned"
              ? state.specs.filter((s) => s.assigneeId && ["approved", "build_queue"].includes(s.status))
              : state.specs.filter((s) => col.statuses.includes(s.status) && !(col.key === "ready" && s.assigneeId));
            return (
              <div key={col.key} className="rounded-xl border border-border bg-card/40 min-h-[300px]">
                    ...
                    return (
                      <Link key={s.id} to="/specs/$id" params={{ id: s.id }}
                        ...
                        <div className="flex items-center justify-between mt-3 text-xs text-muted-foreground">
                          ...
                          {s.gitSyncedAt && <span className="inline-flex items-center gap-1"><GitBranch className="h-3 w-3" />synced</span>}
