  ...
  return (
    <>
      <PageHeader
        ...
        description="Features ready for testing through preview/staging URLs."
      />
        ...
        {items.map((s) => (
          <div key={s.id} className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="p-5">
              ...
              <Link href={`/specs/${s.id}`} className="block">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <Button size="sm" onClick={() => setOpen(s)} className="gap-1.5">
        ...
        <PreviewModal
          ...
          onApprove={() => {
            void setSpecStatus(open.id, "accepted")
              ...
              .catch((error) =>
                toast.error(
                  error instanceof Error ? error.message : "Could not approve preview.",
                ),
              );
          }}
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="sm:max-w-4xl p-0 overflow-hidden">
        <DialogHeader className="sr-only">
          <DialogTitle>Preview of {spec.title}</DialogTitle>
        </DialogHeader>
        <div className="grid md:grid-cols-[1fr_280px]">
          ...
          <div className="border-l border-border bg-card p-4 space-y-4">
            ...
            <div className="space-y-2">
              <Button size="sm" className="w-full gap-1.5" onClick={onApprove}>
                <CheckCircle2 className="h-3.5 w-3.5" />Approve
              </Button>
              <Button
                size="sm"
              ...
              <Button
                size="sm"
                variant="outline"
                className="w-full gap-1.5 text-rose-300 border-rose-500/30 hover:bg-rose-500/10"
                onClick={onReject}
              >
                <X className="h-3.5 w-3.5" />Reject
              </Button>
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {spec.id} {spec.title} will return to Developer Review.
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-2">
          <Textarea
          ...
          {err && (
            <p className="text-xs text-rose-400">
              Please add a reason so the developer knows what to fix.
            </p>
          )}
        </div>
        <DialogFooter>
