...
export default function RoadmapPage(): any {
  ...
  async function openSuggestion() {
    ...
    try {
      ...
      setSuggestion(
        error instanceof Error ? error.message : "Could not load roadmap suggestion.",
      );
    }
  }
  ...
  async function applyPlan() {
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : "Could not apply roadmap suggestions.",
      );
    }
  }
      ...
      <div className="p-6 grid lg:grid-cols-4 gap-4">
        {lanes.map((lane) => {
          ...
          return (
            <div
              key={lane}
              className="flex flex-col rounded-xl border border-border bg-card/40 min-h-[400px]"
            >
                ...
                {items.map((s) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <Link href={`/specs/${s.id}`} className="min-w-0 flex-1">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              AI-generated based on priority and approval status.
            </DialogDescription>
          </DialogHeader>
