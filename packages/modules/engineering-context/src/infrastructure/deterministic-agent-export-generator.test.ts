...
describe('DeterministicAgentExportGenerator', () => {
  ...
  it('warns when spec is not approved', async () => {
    ...
    const result = await generator.generateSpecAgentContext({
      ...
      projectId: 'p1',
      specId: 's1',
      specDetails: { title: 'Test Spec', status: 'draft', priority: 'high' } as unknown as import('../application/ports/agent-export-generator.port').GenerateSpecContextInput['specDetails'],
      context: null as unknown as import('../application/ports/agent-export-generator.port').GenerateSpecContextInput['context'],
      rules: [],
      adrs: [],
